...
public actor GatewayNodeSession {
    ...
    private func handleEvent(_ evt: EventFrame) async {
        ...
        do {
            ...
            let response = await Self.invokeWithTimeout(
                ...
                onInvoke: onInvoke)
            self.logger
                .info("node invoke completed id=\(request.id, privacy: .public) ok=\(response.ok, privacy: .public)")
    ...
    private func sendInvokeResult(request: NodeInvokeRequestPayload, response: BridgeInvokeResponse) async {
        guard let channel = self.channel else { return }
        self.logger
            .info("node invoke result sending id=\(request.id, privacy: .public) ok=\(response.ok, privacy: .public)")

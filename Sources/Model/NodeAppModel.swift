...
final class NodeAppModel {
    ...
    var activeAgentName: String {
        ...
        if resolvedId.isEmpty { return "Nimboclaw" }
        if let match = self.gatewayAgents.first(where: { $0.id == resolvedId }) {
            ...
            if match.id.lowercased() == "main" { return "Nimboclaw" }
            return match.id
        }
        if resolvedId.lowercased() == "main" { return "Nimboclaw" }
        return resolvedId
    }
...
extension NodeAppModel {
                ...
                do {
                    ...
                    try await self.nodeGateway.connect(
                        ...
                        onConnected: { [weak self] in
                            ...
                            GatewayDiagnostics
                                .log("gateway connected host=\(url.host ?? "?") scheme=\(url.scheme ?? "?")")
                            ...
                            await MainActor.run { SignificantLocationMonitor.startIfNeeded(
    ...
    private func makeOperatorConnectOptions(clientId: String, displayName: String?) -> GatewayConnectOptions {

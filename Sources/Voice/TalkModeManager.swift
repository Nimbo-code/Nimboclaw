...
final class TalkModeManager: NSObject {
    ...
    private func buildPrompt(transcript: String) -> String {
        ...
        self.lastInterruptedAtSeconds = nil
        let includeVoiceDirectiveHint = (UserDefaults.standard
            .object(forKey: "talk.voiceDirectiveHint.enabled") as? Bool) ?? true
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: OpenClawAgentEventPayload.self)
            else {
                continue
            }
...
extension TalkModeManager {
    ...
    private static func describeAudioSession() -> String {
        ...
        let outputs = session.currentRoute.outputs.map { "\($0.portType.rawValue):\($0.portName)" }
            .joined(separator: ",")
        let available = session.availableInputs?.map { "\($0.portType.rawValue):\($0.portName)" }
            .joined(separator: ",") ?? ""
...
private final class AudioTapDiagnostics: @unchecked Sendable {
    ...
    func onBuffer(_ buffer: AVAudioPCMBuffer) {
        ...
        GatewayDiagnostics.log(
            "\(self.label) mic: buffers=\(count) frames=\(frames) rate=\(Int(rate))Hz ch=\(ch) rms=\(String(format: "%.4f", resolvedRms)) max=\(String(format: "%.4f", maxRms))")
    }
}
